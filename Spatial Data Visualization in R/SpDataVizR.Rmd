...
```{r googlemap2, fig.width=12, fig.height=6, message = FALSE, cache=TRUE}
library(loa)
...
```{r ggmap, fig.width=6, fig.height=6, message=F, cache=TRUE}
library(ggmap)
...
```{r plotgmap, message=FALSE, cache=TRUE}
...
```{r plotgmap2, message=FALSE, cache=TRUE}
...
```{r googleVis, message=FALSE, cache=TRUE}
library(googleVis)
library(rgdal)
...
```{r leaflet3, message = FALSE, cache=TRUE}
...
```{r choro, message=FALSE, cache=TRUE}
library(Quandl)
